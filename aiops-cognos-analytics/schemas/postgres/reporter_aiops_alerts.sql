...
--       EXAMPLE:    psql -d postgres -f postgres/reporter_aiops_alerts.sql
--------------------------------------------------------------------------------
...
-- CREATE SCHEMA IF NOT EXISTS aiops_reporting;
...
CREATE OR REPLACE FUNCTION ALERTS_AUDIT_INSERT_FN() RETURNS TRIGGER AS $$
BEGIN
        -- start acknowledged procedure
        UPDATE ALERTS_AUDIT_ACK SET 
                enddate = NEW.lastStateChangeTime,
                state = 1
        WHERE
                uuid = NEW.uuid AND
                state = 0 ;
...
CREATE OR REPLACE TRIGGER ALERTS_AUDIT_INSERT
AFTER INSERT ON ALERTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( NEW.lastStateChangeTime IS NOT NULL )
        EXECUTE FUNCTION ALERTS_AUDIT_INSERT_FN();

CREATE OR REPLACE FUNCTION ALERTS_AUDIT_UPDATE_SEVERITY_FN() RETURNS TRIGGER AS $$
BEGIN
        -- start severity procedure
        UPDATE ALERTS_AUDIT_SEVERITY SET
                enddate = NEW.lastStateChangeTime,
...
CREATE OR REPLACE TRIGGER ALERTS_AUDIT_UPDATE_SEVERITY
        AFTER UPDATE ON ALERTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.severity <> NEW.severity AND NEW.lastStateChangeTime IS NOT NULL )
        EXECUTE FUNCTION ALERTS_AUDIT_UPDATE_SEVERITY_FN();

CREATE OR REPLACE FUNCTION ALERTS_AUDIT_UPDATE_OWNER_FN() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER ALERTS_AUDIT_UPDATE_OWNER
        AFTER UPDATE ON ALERTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.owner <> NEW.owner AND NEW.lastStateChangeTime IS NOT NULL )
        EXECUTE FUNCTION ALERTS_AUDIT_UPDATE_OWNER_FN();

CREATE OR REPLACE FUNCTION ALERTS_AUDIT_UPDATE_TEAM_FN() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER ALERTS_AUDIT_UPDATE_TEAM
        AFTER UPDATE ON ALERTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.team <> NEW.team AND NEW.lastStateChangeTime IS NOT NULL )
        EXECUTE FUNCTION ALERTS_AUDIT_UPDATE_TEAM_FN();

CREATE OR REPLACE FUNCTION ALERTS_AUDIT_UPDATE_ACK_FN() RETURNS TRIGGER AS $$
BEGIN
        -- start acknowledged procedure
        UPDATE ALERTS_AUDIT_ACK SET 
                enddate = NEW.lastStateChangeTime,
...
CREATE TRIGGER ALERTS_AUDIT_UPDATE_ACK
        AFTER UPDATE ON ALERTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.acknowledged <> NEW.acknowledged AND NEW.lastStateChangeTime IS NOT NULL )
        EXECUTE FUNCTION ALERTS_AUDIT_UPDATE_ACK_FN();
...
CREATE OR REPLACE VIEW ALERTS_STATUS_VW (
        severity,
        businessCriticality,
        state,
        summary,
        ...
        sender,
        resource,
        (REPLACE(CAST(firstOccurrenceTime AS VARCHAR),'.',':')),
        (REPLACE(CAST(lastOccurrenceTime AS VARCHAR),'.',':')),
        runbooks,
        case 
...
CREATE OR REPLACE VIEW ALERTS_AUDIT (
        id, 
        lastStateChangeTime, 
        valueAck, 
        valueSeverity, 
