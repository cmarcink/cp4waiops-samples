...
--       EXAMPLE:   psql -d postgres -f postgres/reporter_aiops_incidents.sql
--------------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION INCIDENTS_AUDIT_INSERT_FN() RETURNS TRIGGER AS $$
BEGIN
        UPDATE INCIDENTS_AUDIT_STATE SET 
                enddate = NEW.lastChangedTime,
                complete = 1
        WHERE
                uuid = NEW.uuid
        AND
...
CREATE OR REPLACE TRIGGER INCIDENTS_AUDIT_INSERT
    AFTER INSERT ON INCIDENTS_REPORTER_STATUS
    FOR EACH ROW 
    WHEN ( NEW.lastChangedTime IS NOT NULL )
    EXECUTE FUNCTION INCIDENTS_AUDIT_INSERT_FN();

CREATE OR REPLACE FUNCTION INCIDENTS_AUDIT_UPDATE_PRIORITY_FN() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER INCIDENTS_AUDIT_UPDATE_PRIORITY
        AFTER UPDATE ON INCIDENTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.priority <> NEW.priority AND NEW.lastChangedTime IS NOT NULL )
        EXECUTE FUNCTION INCIDENTS_AUDIT_UPDATE_PRIORITY_FN();

CREATE OR REPLACE FUNCTION INCIDENTS_AUDIT_UPDATE_OWNER_FN() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER INCIDENTS_AUDIT_UPDATE_OWNER
    AFTER UPDATE ON INCIDENTS_REPORTER_STATUS
    FOR EACH ROW 
    WHEN ( OLD.owner <> NEW.owner AND NEW.lastChangedTime IS NOT NULL )
    EXECUTE FUNCTION INCIDENTS_AUDIT_UPDATE_OWNER_FN();


CREATE OR REPLACE FUNCTION INCIDENTS_AUDIT_UPDATE_TEAM_FN() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER INCIDENTS_AUDIT_UPDATE_TEAM
        AFTER UPDATE ON INCIDENTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.team <> NEW.team AND NEW.lastChangedTime IS NOT NULL )
        EXECUTE FUNCTION INCIDENTS_AUDIT_UPDATE_TEAM_FN();

CREATE OR REPLACE FUNCTION INCIDENTS_AUDIT_UPDATE_STATE_FN() RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE TRIGGER INCIDENTS_AUDIT_UPDATE_STATE
        AFTER UPDATE ON INCIDENTS_REPORTER_STATUS
        FOR EACH ROW 
        WHEN ( OLD.state <> NEW.state AND NEW.lastChangedTime IS NOT NULL )
        EXECUTE FUNCTION INCIDENTS_AUDIT_UPDATE_STATE_FN();
...
CREATE OR REPLACE VIEW INCIDENTS_STATUS_VW (
        priority,
        state,
        id,
        title,
        ...
        title,
        alerts,
        REPLACE(CAST(createdTime AS VARCHAR),'.',':'),
        team,
        owner
FROM 
        INCIDENTS_REPORTER_STATUS;

CREATE OR REPLACE VIEW INCIDENTS_AUDIT (
        id, 
        lastChangedTime, 
        valueState, 
        valuePriority, 
